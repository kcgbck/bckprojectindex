...
## 1. 프로젝트 개요
- 해양경찰 및 관련 업무에 필요한 101개 주요 법령/행정규칙 데이터를 국가법령정보센터 Open API를 통해 자동 수집·관리합니다.
...
## 3. 핵심 규칙 및 주의사항
- **법령 ID 변경 시 주의**: `update_laws.js`의 `LAW_LIST`에 정의된 API URL의 Law ID 및 타겟 유형(`eflaw`, `admrul` 등)이 정확한지 `validate_laws.js`로 항상 검증합니다.
